...
contract SharedBike{
    ...
    struct user_info{
        ...
        uint time;//时间戳
        mapping(address => address) find_bike;//通过车的地址找到租车人是谁。前面一个address是车的地址，后一个address是用户地址
    ...
    Library.admin_info private _bikeself;//调用这里面的结构体中的mapping（address => uint）去表明车的状态；
   //车的状态 0 可以骑，1 已经被他人骑了 2 车已经被报修了
    ...
    function AddBike(address bikeaddress)public returns(bytes32 _bikesecretkey){
        _bikeself.admin_purview[bikeaddress]=0;//上架一部车时，车的状态默认为0
    }
    function  BorrowBikes(bytes32 _usersecretkey,address bikeaddress)public returns(string){
        ...
        //用户骑车，通过这个地址 ，去查验车的状态
        if(_bikeself.admin_purview[bikeaddress]==1)
        {
            revert("车已经被人骑走了，再去找找其他车吧");
        }
        //车被报修了
             if(_bikeself.admin_purview[bikeaddress]==2)
        {
            revert("该车已经被报修了，再去找找其他车吧");
        }
        //可以租借，首先改变车状态,然后把这部车的地址和租借走的用户的地址绑定起来
               if(_bikeself.admin_purview[bikeaddress]==0)
        {
            //改变车的状态
            _bikeself.admin_purview[bikeaddress]=1;
            //把这部车的地址和租借走的用户的地址绑定起来
            user_dic[_usersecretkey].find_bike[bikeaddress]=user_dic[_usersecretkey].useraddress;
    ...
    //归还单车函数
    function ReturnBikes(bytes32 _usersecretkey,address bikeaddress)public returns(string){
        //改变车的状态
        _bikeself.admin_purview[bikeaddress]=0;
         //将用户可借车状态修改为true，可以继续租车了
        user_dic[_usersecretkey].user_status[msg.sender]=true;
    }
    //Repair报修函数，用户在骑车时碰到车有问题可以报修，该车其他用户将不再能骑
    function Repair(address bikeaddress)public returns(string){
        //将车的状态改为报修状态
        _bikeself.admin_purview[bikeaddress]=2;
    }
    //管理员功能:给予停车规范的车加分,上传车的地址 ，通过这部车的地址去找到租这部车的人的地址，再通过租车人的地址去加分
    function Reward(bytes32 _usersecretkey, address _bikeaddress)public returns(bool){
       address addressuser=user_dic[_usersecretkey].find_bike[_bikeaddress];
       user_dic[_usersecretkey].bike_points[addressuser]+=10;
       return true;
    }
    //管理员功能：给予不规范停车的扣分， ，通过这部车的地址去找到租这部车的人的地址，再通过租车人的地址去扣分
    function Punishment(bytes32 _usersecretkey,address _bikeaddress)public returns(bool){
        address addressuser=user_dic[_usersecretkey].find_bike[_bikeaddress];
        user_dic[_usersecretkey].bike_points[addressuser]-=10;
        return true;
    }
